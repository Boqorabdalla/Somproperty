                ...
                <div class="row px-lg-4 px-md-4 px-3 py-3">
                    <div class="col-md-3">
                        <x-forms.text :fieldLabel="'PO Number'"
                            ...
                            :fieldValue="$purchaseOrder->po_number"/>
                    </div>
                    <div class="col-md-3">
                        <x-forms.datepicker fieldId="order_date"
                            :fieldLabel="'Order Date'" fieldName="order_date"
                            :fieldPlaceholder="__('placeholders.date')"
                            :fieldValue="$purchaseOrder->order_date ? $purchaseOrder->order_date->format(company()->date_format) : now(company()->timezone)->format(company()->date_format)"/>
                    </div>
                    <div class="col-md-3">
                        <x-forms.datepicker fieldId="expected_delivery"
                            :fieldLabel="'Expected Delivery'" fieldName="expected_delivery"
                            :fieldPlaceholder="__('placeholders.date')"
                            :fieldValue="$purchaseOrder->expected_delivery ? $purchaseOrder->expected_delivery->format(company()->date_format) : ''"/>
                    </div>
                    <div class="col-md-3">
                        <x-forms.select fieldId="currency_id" :fieldLabel="'Currency'"
                            fieldName="currency_id" search="true">
                            @foreach ($currencies as $currency)
                                <option @selected($currency->id == $purchaseOrder->currency_id) value="{{ $currency->id }}">
                    ...
                    <div class="col-md-4">
                        <x-forms.select fieldId="vendor_id" :fieldLabel="'Vendor'"
                            ...
                            @foreach ($vendors as $vendor)
                                <option @selected($vendor->id == $purchaseOrder->vendor_id) value="{{ $vendor->id }}">{{ $vendor->name }}</option>
                            @endforeach
                        </x-forms.select>
                    </div>
                    <div class="col-md-4">
                        <x-forms.select fieldId="project_id" :fieldLabel="'Project'"
                            ...
                            @foreach ($projects as $project)
                                <option @selected($project->id == $purchaseOrder->project_id) value="{{ $project->id }}">{{ $project->project_name }}</option>
                            @endforeach
                        </x-forms.select>
                    </div>
                </div>
                                        ...
                                        <tr>
                                            <td class="border-bottom-0 btrr-mbl btlr">
                                                ...
                                                <textarea class="form-control f-14 border-0 w-100" name="item_summary[]"
                                                    placeholder="@lang('placeholders.invoices.description')" rows="2">{{ $item->item_summary }}</textarea>
                                            </td>
                                        ...
                                        <tbody>
                                            <tr>
                                                <td colspan="2" class="border-top-0 text-dark-grey">
                                                    Sub Total</td>
                                                <td width="30%" class="border-top-0 sub-total">{{ number_format((float) $purchaseOrder->sub_total, 2, '.', '') }}</td>
                                                <input type="hidden" class="sub-total-field" name="sub_total" value="{{ $purchaseOrder->sub_total }}">
                                            </tr>
                                            <tr>
                                                            ...
                                                            <tr>
                                                                <td width="70%" class="c-inv-sub-padding">
                                                                    <input type="number" min="0" name="discount"
                                                                        ...
                                                                        placeholder="0" value="{{ $purchaseOrder->discount }}">
                                                                </td>
                                                                <td width="30%" align="left" class="c-inv-sub-padding">
                                                                    <div class="select-others select-tax height-35 rounded border-0">
                                                                        <select class="form-control select-picker"
                                                                            ...
                                                                            <option @selected($purchaseOrder->discount_type == 'percentage') value="percentage">%</option>
                                                                            <option @selected($purchaseOrder->discount_type == 'fixed') value="fixed">Amount</option>
                                                                        </select>
                                                                    </div>
                                                                </td>
                                                            </tr>
                                                ...
                                                <td><span id="discount_amount">{{ number_format((float) $purchaseOrder->discount, 2, '.', '') }}</span></td>
                                            </tr>
                                            <tr>
                                                <td>Tax</td>
                                                <td colspan="2" class="p-0 border-0">
                                                    <table width="100%" id="purchase-order-taxes">
                                                        <tr>
                                                            <td colspan="2"><span class="tax-percent">{{ number_format((float) $purchaseOrder->tax, 2, '.', '') }}</span></td>
                                                        </tr>
                                                    </table>
                                                </td>
                                            </tr>
                                            <tr class="bg-amt-grey f-16 f-w-500">
                                                ...
                                                <td><span class="total">{{ number_format((float) $purchaseOrder->total, 2, '.', '') }}</span></td>
                                                <input type="hidden" class="total-field" name="total" value="{{ $purchaseOrder->total }}">
                                            </tr>
                                        </tbody>
                                    </table>
                                </td>
                ...
                <div class="d-flex flex-wrap px-lg-4 px-md-4 px-3 py-3">
                    <div class="col-md-6 col-sm-12 c-inv-note-terms p-0 mb-lg-0 mb-md-0 mb-3">
                        ...
                        <textarea class="form-control" name="notes" id="notes" rows="4"
                            placeholder="@lang('placeholders.notes')">{{ $purchaseOrder->notes }}</textarea>
                    </div>
                    <div class="col-md-6 col-sm-12 p-0 c-inv-note-terms">
                        ...
                        <textarea class="form-control" name="terms" id="terms" rows="4"
                            placeholder="@lang('placeholders.terms')">{{ $purchaseOrder->terms }}</textarea>
                    </div>
                </div>

                <x-form-actions>
                    <x-forms.button-primary id="update-purchase-order-form" class="mr-3" icon="check">Save
...
<script>
    $(document).ready(function () {
        ...
        $('#update-purchase-order-form').click(function () {
            const url = "{{ route('purchase-orders.update', $purchaseOrder->id) }}";
            ...
            $.easyAjax({
                ...
                disableButton: true,
                blockUI: true,
                buttonSelector: "#update-purchase-order-form",
                data: $('#save-purchase-order-data-form').serialize() + '&_method=PUT',
