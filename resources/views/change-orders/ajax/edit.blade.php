<div class="row">
    <div class="col-sm-12">
        <x-form id="save-change-order-form" method="PUT">
            <div class="bg-white rounded add-client">
                ...
                <div class="p-20 row">
                    ...
                    <div class="col-md-4">
                        <x-forms.text class="mr-0 mr-lg-2 mr-md-2"
                                      ...
                                      :fieldValue="$changeOrder->change_order_number"/>
                    </div>
                        ...
                        <x-forms.input-group>
                            <select class="form-control select-picker" name="project_id" id="project_id"
                                ...
                                @foreach ($projects as $project)
                                    <option @selected($changeOrder->project_id == $project->id)
                        ...
                        <x-forms.input-group>
                            <select class="form-control select-picker" name="currency_id" id="currency_id"
                                ...
                                @foreach ($currencies as $currency)
                                    <option @selected($changeOrder->currency_id == $currency->id)
                                    value="{{ $currency->id }}">
                    ...
                    <div class="col-md-12">
                        <x-forms.text class="mr-0 mr-lg-2 mr-md-2"
                                      ...
                                      :fieldValue="$changeOrder->title"/>
                    </div>

                    <div class="col-md-12">
                        <x-forms.label class="my-3" fieldId="description" :fieldLabel="'Description'">
                        </x-forms.label>
                        <div id="description">{!! $changeOrder->description !!}</div>
                        <textarea name="description" id="description-text" class="d-none">{!! $changeOrder->description !!}</textarea>
                    </div>

                    <div class="col-md-12">
                        <x-forms.label class="my-3" fieldId="reason" :fieldLabel="'Reason'">
                        </x-forms.label>
                        <div id="reason">{!! $changeOrder->reason !!}</div>
                        <textarea name="reason" id="reason-text" class="d-none">{!! $changeOrder->reason !!}</textarea>
                    </div>
                ...
                <div class="p-20 row">
                    <div class="col-md-12">
                        ...
                        <div id="sortable">
                            @forelse ($changeOrder->items as $key => $item)
                                            ...
                                            <tr>
                                                ...
                                                <td class="border-bottom-0">
                                                    <input type="number" min="1"
                                                        class="form-control f-14 border-0 w-100 text-right quantity mt-3" value="{{ $item->quantity }}"
                                                        name="quantity[]">
                                                </td>
                                                <td class="border-bottom-0">
                                    ...
                                    <table width="100%">
                                        <tbody>
                                            <tr>
                                                ...
                                                <td width="30%" class="border-top-0 sub-total">{{ number_format((float) ($changeOrder->sub_total ?? 0), 2, '.', '') }}</td>
                                                <input type="hidden" class="sub-total-field" name="sub_total" value="{{ $changeOrder->sub_total ?? 0 }}">
                                            </tr>
                                            <tr>
                                                ...
                                                <td>
                                                    <input type="number" min="0" step="0.01" name="tax"
                                                        ...
                                                        placeholder="0.00" value="{{ $changeOrder->tax ?? 0 }}">
                                                </td>
                                            </tr>
                                            <tr class="bg-amt-grey f-16 f-w-500">
                                                <td>Total</td>
                                                <td><span class="total">{{ number_format((float) ($changeOrder->total ?? 0), 2, '.', '') }}</span></td>
                                                <input type="hidden" class="total-field" name="total" value="{{ $changeOrder->total ?? 0 }}">
                                            </tr>
                                        </tbody>
                                    </table>
                                </td>
...
<script>
    $(document).ready(function() {
        ...
        $('#save-change-order-form-btn').click(function() {
            ...
            const url = "{{ route('change-orders.update', $changeOrder->id) }}";
