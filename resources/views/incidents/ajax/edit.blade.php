<div class="row">
    <div class="col-sm-12">
        <x-form id="save-incident-form" method="PUT">
                ...
                <div class="p-20 row">
                        ...
                        <x-forms.input-group>
                            <select class="form-control select-picker" name="project_id" id="project_id"
                                ...
                                @foreach ($projects as $project)
                                    <option @selected($incident->project_id == $project->id)
                        ...
                        <x-forms.input-group>
                            <select class="form-control select-picker" name="category_id" id="category_id"
                                ...
                                @foreach ($categories as $category)
                                    <option @selected($incident->category_id == $category->id)
                    ...
                    <div class="col-md-4">
                        <x-forms.datepicker fieldId="incident_date" fieldRequired="true"
                                            ...
                                            :fieldValue="$incident->incident_date->format(company()->date_format)"/>
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <x-forms.input-group>
                            <select class="form-control select-picker" name="severity" id="severity">
                                <option @selected($incident->severity == 'minor') value="minor">Minor</option>
                                <option @selected($incident->severity == 'major') value="major">Major</option>
                                <option @selected($incident->severity == 'critical') value="critical">Critical</option>
                            </select>
                        </x-forms.input-group>
                    </div>
                    ...
                    <div class="col-md-12">
                        ...
                        <textarea name="description" id="description-text" class="d-none">{!! $incident->description !!}</textarea>
                    </div>

                    <div class="col-md-12">
                        <x-forms.label class="my-3" fieldId="root_cause" :fieldLabel="'Root Cause'">
                        </x-forms.label>
                        <div id="root_cause">{!! $incident->root_cause !!}</div>
                        <textarea name="root_cause" id="root_cause-text" class="d-none">{!! $incident->root_cause !!}</textarea>
                    </div>

                    <div class="col-md-12">
                        <x-forms.label class="my-3" fieldId="corrective_action" :fieldLabel="'Corrective Action'">
                        </x-forms.label>
                        <div id="corrective_action">{!! $incident->corrective_action !!}</div>
                        <textarea name="corrective_action" id="corrective_action-text" class="d-none">{!! $incident->corrective_action !!}</textarea>
                    </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#save-incident-form-btn').click(function() {
            ...
            const url = "{{ route('incidents.update', $incident->id) }}";
