<div class="row">
    <div class="col-sm-12">
        <x-form id="save-material-data-form" method="PUT">
            <div class="add-client bg-white rounded">
                <h4 class="mb-0 p-20 f-21 font-weight-normal border-bottom-grey">
                    @lang('app.menu.editMaterial')</h4>
                <div class="row p-20">
                    <div class="col-lg-4 col-md-6">
                        <x-forms.text fieldId="name" fieldLabel="Name" fieldName="name" fieldRequired="true"
                            :fieldPlaceholder="__('placeholders.name')" :fieldValue="$material->name">
                        </x-forms.text>
                    </div>

                    <div class="col-lg-4 col-md-6">
                        <x-forms.text fieldId="sku" fieldLabel="SKU" fieldName="sku"
                            :fieldPlaceholder="__('placeholders.sku')" :fieldValue="$material->sku">
                        </x-forms.text>
                    </div>

                    <div class="col-lg-4 col-md-6">
                        <x-forms.text fieldId="unit" fieldLabel="Unit" fieldName="unit"
                            :fieldPlaceholder="__('placeholders.unit')" :fieldValue="$material->unit">
                        </x-forms.text>
                    </div>
                    ...
                    <div class="col-lg-4 col-md-6">
                        <x-forms.select fieldId="unit_type" fieldLabel="Unit Type" fieldName="unit_type"
                            ...
                            @foreach ($unitTypes as $unitType)
                                <option value="{{ $unitType->id }}" @if ($unitType->id == $material->unit_type) selected @endif>{{ $unitType->unit_type }}</option>
                            @endforeach
                        </x-forms.select>
                    </div>
                    ...
                    <div class="col-lg-4 col-md-6">
                        <x-forms.select fieldId="category_id" fieldLabel="Category"
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}" @if ($category->id == $material->category_id) selected @endif>{{ $category->category_name }}</option>
                            @endforeach
                        </x-forms.select>
                    </div>
                    ...
                    <div class="col-lg-4 col-md-6">
                        <x-forms.number class="mr-0 mr-lg-2 mr-md-2" fieldLabel="Unit Price"
                            ...
                            :fieldValue="$material->unit_price" />
                    </div>
                    ...
                    <div class="col-lg-4 col-md-6">
                        <x-forms.number class="mr-0 mr-lg-2 mr-md-2" fieldLabel="Current Stock"
                            ...
                            :fieldValue="$material->current_stock" />
                    </div>
                    ...
                    <div class="col-lg-4 col-md-6">
                        <x-forms.select fieldId="project_id" fieldLabel="Project" fieldName="project_id"
                            ...
                            @foreach ($projects as $project)
                                <option value="{{ $project->id }}" @if ($project->id == $material->project_id) selected @endif>{{ $project->project_name }}</option>
                            @endforeach
                        </x-forms.select>
                    </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#save-material-form').click(function() {
            const url = "{{ route('materials.update', $material->id) }}";
